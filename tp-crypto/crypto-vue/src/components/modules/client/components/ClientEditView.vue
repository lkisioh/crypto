...
async function BuscarDatosApi() {
  ...
  if (response.ok) {
    alert('Search succesfully');
    editedClient.value = await response.json();
  } else {
...
<template>
  ...
  <div class="form-container">
   ...
   <Form :validation-schema="schema" @submit="EditarDatosApi" id="formulario-carga" class="form-box">
    <label>
      Name:
    <Field v-model="editedClient.name" type="text" name="name" id="name" class="input-field"/>
    </label>
    ...
    <label>
      Email:
      <Field v-model="editedClient.email" type="text" name="email" id="email" class="input-field"/>
    </label>
